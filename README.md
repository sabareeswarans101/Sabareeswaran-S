<h1 align="center">SABAREESWARAN S</h1>
<p align="center"><strong>BE.Computer Science Student | Aspiring Software Developer</strong></p>

<p align="center">
  <a href="https://github.com/sabareeswans101"><img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" /></a>
  <a href="https://www.linkedin.com/in/sabareeswaran-sundaramoorthy-574787304/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
  <a href="https://sundarsabaree.github.io/portfolio/"><img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white" /></a>
</p>

<p align="center">📍 Avinashi, Tiruppur, Tamil Nadu, India</p>

<br>

## 👋 Introduction

Hi, I'm Sabareeswaran S — a Computer Science student developing my programming skills through practical, real-world projects. I'm interested in software development, exploring Data Science and AI, learning Web Development, and experimenting with Game Development, and I'm continuously working on improving my problem-solving and programming fundamentals.

<br>

## 👨‍💻 About Me

- 🎓 BE.Computer Science student
- 💻 Java Development
- 🌐 Web Development
- 📊 Data Science
- 🤖 Artificial Intelligence
- 🧠 Machine Learning
- 🎮 Game Development
- 🧩 Problem Solving
- 🏆 Coding practice & DSA
- 📢 Digital Marketing experience

<br>

## 📚 Currently Learning

`Advanced Java` `Data Structures & Algorithms` `Python` `Data Science` `Machine Learning` `Artificial Intelligence` `Web Development` `SQL` `Git & GitHub` `Unity / Game Development`

## 🔭 Future Learning Goals

`Deep Learning` `Full Stack Development` `React` `Node.js` `Spring Boot` `Cloud Computing` `APIs` `Docker` `DevOps` `Advanced Database Systems` `Open Source Development`

<br>

## 🛠️ Tech Stack

**Languages**

![Java](https://img.shields.io/badge/Java-Intermediate-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![Python](https://img.shields.io/badge/Python-Intermediate-3776AB?style=flat-square&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-Learning-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![HTML5](https://img.shields.io/badge/HTML5-Learning-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-Learning-1572B6?style=flat-square&logo=css3&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-Learning-4479A1?style=flat-square&logo=mysql&logoColor=white)

**Data Science & AI**

![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=flat-square&logo=plotly&logoColor=white)
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-Exploring-F7931E?style=flat-square&logo=scikitlearn&logoColor=white)
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-FF6F00?style=flat-square&logo=tensorflow&logoColor=white)

**Development & Databases**

![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=flat-square&logo=visualstudiocode&logoColor=white)
![Unity](https://img.shields.io/badge/Unity-000000?style=flat-square&logo=unity&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)

<br>

## 🚀 Featured Projects

<table width="100%">
<tr>
<td width="50%" valign="top">

**🩸 Blood Donation Management System**
A system designed to help connect blood donors with people who urgently need blood and improve the process of finding suitable donors.
`Web Development`
🔗 [Repository](YOUR_REPOSITORY_URL)

</td>
<td width="50%" valign="top">

**🅿️ Campus Parking Management System**
A smart campus parking concept designed to make vehicle parking more organized and efficient.
`Web Development`
🔗 [Repository](YOUR_REPOSITORY_URL)

</td>
</tr>
<tr>
<td width="50%" valign="top">

**📊 Data Science Projects**
Projects covering data collection, preprocessing, cleaning, exploratory data analysis, visualization, and basic machine learning.
`Python` `Pandas` `NumPy`
🔗 [Repository](YOUR_REPOSITORY_URL)

</td>
<td width="50%" valign="top">

**🎮 Unity 2D Game**
A dark-fantasy 2D game concept built in Unity, exploring character design, animation, movement, environments, and core game mechanics.
`Unity`
🔗 [Repository](YOUR_REPOSITORY_URL)

</td>
</tr>
</table>

<sub>Repository and live-demo links above are placeholders — replace `YOUR_REPOSITORY_URL` with the actual links.</sub>

<br>

## 💼 Experience

**Data Science Intern** — Getin Technologies
`01 June 2026 – 17 June 2026`

Data Collection · Data Preprocessing · Data Cleaning · Exploratory Data Analysis · Data Visualization · Basic Machine Learning · Model Training · Model Evaluation · Feature Selection

`Python` `Pandas` `NumPy` `Matplotlib`

---

**AI-Powered Digital Marketing** — Jewel Jothi Technologies

AI-Assisted Content Creation · Social Media Marketing · Branding · Digital Marketing · Campaign Planning · AI-Assisted Graphic Design

<br>

## 🎓 Education

**KGISL Institute of Technology**
BE.Computer Science — Tamil Nadu, India

<br>

## 🧩 Problem Solving & Coding

Practicing core programming and problem-solving skills, including:

`Java Problem Solving` `Arrays` `Strings` `Matrix Problems` `Searching` `Sorting` `Data Structures` `Algorithms` `Logical Problem Solving`

<sub>Coding-platform usernames not yet added — profile links will be included here once available.</sub>

<br>

## 🏅 Certifications

Certifications will be added as I continue my learning journey.

<br>

## 📊 GitHub Statistics

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=sabareeswans101&show_icons=true&theme=dark&hide_border=true&count_private=true" width="48%" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=sabareeswans101&layout=compact&theme=dark&hide_border=true" width="40%" />
</p>

## 📈 GitHub Activity

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=sabareeswans101&theme=dark&hide_border=true" width="60%" />
</p>

<p align="center">
  <a href="https://github.com/sabareeswans101">View my full GitHub contribution graph →</a>
</p>

<br>

## 🎯 Career Goals

- Grow as a software developer
- Strengthen Java and programming fundamentals
- Build real-world projects
- Develop strong Data Science and AI skills
- Improve problem-solving ability
- Explore full-stack development
- Contribute to meaningful projects
- Prepare for software development opportunities

<br>

## 🤝 Let's Connect

I'm open to connecting with developers, recruiters, and teams working on interesting technology projects — including internships, software development opportunities, collaborations, and open-source work.

<p align="center">
  <a href="https://github.com/sabareeswans101"><img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" /></a>
  <a href="https://www.linkedin.com/in/sabareeswaran-sundaramoorthy-574787304/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
  <a href="https://github.com/Sundarsabaree/portfolio"><img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white" /></a>
  <a href="mailto:YOUR_EMAIL_HERE"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" /></a>
</p>

<br>

<p align="center">Thanks for visiting my profile! Let's connect and build something meaningful together. 🚀</p>

<p align="center">
  <a href="https://github.com/sabareeswans101">GitHub</a> |
  <a href="https://www.linkedin.com/in/sabareeswaran-sundaramoorthy-574787304/">LinkedIn</a> |
  <a href="https://github.com/Sundarsabaree/portfolio">Portfolio</a>
</p>
